...
template<typename DataType, int Size>
size_t RingBuffer<DataType, Size>::readSafe(DataType* outData, int inSize)
{
    const int available = getLength();
    const size_t length = std::min(available, inSize);
...
template<typename DataType, int Size>
size_t RingBuffer<DataType, Size>::readOverflow(DataType* outData, int inSize)
{
    for (int i = 0; i < inSize; ++i)
    {
        outData[i] = read();
...
template<int BufferSize>
void SerialMock<BufferSize>::write(const uint8* inData, int inLength)
{
    mTxBuffer.write(inData, inLength);
}

// --

template<int BufferSize>
uint8 SerialMock<BufferSize>::peek()
{
...
template<int BufferSize>
size_t SerialMock<BufferSize>::readBytes(uint8* outBuffer, int inLength)
{
    return mRxBuffer.readSafe(outBuffer, inLength);
}
...
{
    mRxBuffer.clear();
    const int size = mTxBuffer.getLength();
